                ...
                <div class="navbar-center">
                    <div class="flex px-4 py-3 rounded-2xl border-2 overflow-hidden max-w-md mx-auto font-[sans-serif]">
                        <form method="GET" action="{{ route('cariayam') }}" id="filter-form">
                            <input type="text" wire:model="searchTerm" name="q" placeholder="Cari Ayam" class="w-full outline-none bg-transparent text-gray-600 text-sm" />
                        </form>
                ...
                <div class="navbar-end">
                    ...
                    <x-dropdown>
                        <div class="bg-pink rounded-xl mt-2 mb-2 p-2">
                            <p class="font-semibold text-lg text-white">{{ Auth::user()->nama_depan }} {{ Auth::user()->nama_belakang }}</p>
                            <p class="text-base text-white">{{ Auth::user()->email }}</p>
                        </div>
                        <x-slot:trigger>
                        ...
                        <x-menu-item class="hover:bg-pink hover:text-white active:bg-heading" title="Riwayat Pembelian" />
                        <x-menu-item class="hover:bg-pink hover:text-white active:bg-heading" title="Pengaturan" />
                        <x-menu-item class="hover:bg-pink hover:text-white active:bg-heading" title="Logout" href="{{route('logout')}}" />
                    </x-dropdown>
                    @endauth
                </div>
            </div>
